...
const moduleTrip = {
  ...
  actions: {
    ...
    [`TRIP.SET_DIRECTIONS`]({commit}, payload) {
      ...
      commit(SET_CONTAINMENT_POLYGON, payload.bounds.toJSON());
    },
  ...
  getters: {
    ...
    tripSummaryData(state) {
      const addrComp = Vue.filter('pluckAddressComponent');
      return {
        ...
        originAddress: {
          ...
          streetNumber: +addrComp(get(state, '_origin', {}), 'street_number'),
          streetName: addrComp(get(state, '_origin', {}), 'route'),
          city: addrComp(get(state, '_origin', {}), 'locality'),
          state: addrComp(get(state, '_origin', {}),
            'administrative_area_level_1'
          ),
          zip: +addrComp(get(state, '_origin', {}), 'postal_code')
        },
        ...
        destinationAddress: {
          ...
          streetNumber: +addrComp(get(state, '_destination', {}),
            'street_number'
          ),
          streetName: addrComp(get(state, '_destination', {}), 'route'),
          city: addrComp(get(state, '_destination', {}), 'locality'),
          state: addrComp(get(state, '_destination', {}),
            'administrative_area_level_1'
          ),
          zip: +addrComp(get(state, '_destination', {}), 'postal_code')
        },
